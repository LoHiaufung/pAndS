          ...
          and to record the limit on queue sizes. */
{
  ...
  {
     Error_code result;
     if (m_landing.size() < queue_limit)
        result = m_landing.append(current);
     else
        result = fail;
     num_land_requests++;
     if (result != success)
  ...
  {
      Error_code result;
       if (m_takeoff.size() < queue_limit)
       result = m_takeoff.append(current);
      else
         result = fail;
       ...
       Uses: class Extended_queue. */
{
 ...
 if ( !m_landing.empty()) {
    m_landing.retrieve(moving);
    land_wait += time - moving.started();
    num_landings++;
    in_progress = land;
    m_landing.serve();
 }
 else if (!m_takeoff.empty()) {
    m_takeoff.retrieve(moving);
